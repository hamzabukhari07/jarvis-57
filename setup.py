...
Two things it deliberately does NOT install:
  * the optional local wake word ("Hey Jarvis") â€” one-click, opt-in, from
    âš™ â†’ WAKE WORD inside the app;
  * anything for the avatar â€” the holographic head renders in software on the
...
def _check_python() -> None:
    ...
    if v > MAX_PY:
        ...
        # so plainly.
        print(f"\nâš ï¸  Python {v[0]}.{v[1]} is newer than the "
              f"{MAX_PY[0]}.{MAX_PY[1]} this is tested on. Continuing â€” if a "
              f"package has no wheel yet, install Python "
              f"{MAX_PY[0]}.{MAX_PY[1]} and run setup with that.")
        return
    if v < MIN_PY:
        print(f"\nâŒ Python {v[0]}.{v[1]} detected â€” JARVIS needs at "
...
def main() -> None:
    ...
    if OS == "Windows":
        try:
            ...
            print(
                "\nâš ï¸  pywin32 did not register correctly â€” desktop-shortcut "
        ...
        print(
            "\nâ„¹ï¸  Linux note â€” a few voice-controlled OS actions shell out to "
            "native tools. Install the ones you'll use via your package manager:\n"
            "    â€¢ volume      â†’ pulseaudio-utils   (pactl)\n"
            "    â€¢ brightness  â†’ brightnessctl\n"
            "    â€¢ reminders   â†’ systemd (systemd-run) or 'at'\n"
            "    â€¢ open URLs   â†’ xdg-utils          (xdg-open)"
        )
    elif OS == "Darwin":
        print(
            "\nâ„¹ï¸  macOS note â€” volume, brightness and reminders use the built-in "
    ...
    print("   3) (Optional) Enable 'Hey Jarvis' from âš™ â†’ WAKE WORD.")
